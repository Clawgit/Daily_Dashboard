...
export const netflixService = {
  async getTop10(category: string = 'Films', country: string = 'India', signal?: AbortSignal): Promise<NetflixDataResponse> {
    try {
      const params = new URLSearchParams({
        category,
        country,
      });
      return await fetchApi<NetflixDataResponse>(`/netflix?${params.toString()}`, { signal, timeoutMs: 6000 });
    } catch (err: any) {
