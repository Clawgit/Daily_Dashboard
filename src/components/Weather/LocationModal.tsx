...
export const LocationModal: React.FC<LocationModalProps> = ({ isOpen, onClose, onSelectLocation }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-lg bg-[#0f172a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="p-4 border-b border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <MapPin className="w-5 h-5 text-cyan-400" />
            <h3 className="text-base font-semibold text-white">Select Weather Location</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 border-b border-white/10">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search worldwide city (e.g., Tokyo, Berlin, San Francisco)..."
              autoFocus
              className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all"
            />
            {isSearching && (
        ...
        <div className="p-4 overflow-y-auto space-y-4 flex-1">
          {results.length > 0 ? (
              ...
              <div className="space-y-1.5">
                {results.map((loc) => (
                  <button
                    ...
                    className="w-full text-left p-3 rounded-xl hover:bg-white/5 transition-all flex items-center justify-between group border border-transparent hover:border-white/10"
                  >
                    <div>
            ...
            <div>
              <div className="text-xs font-mono uppercase tracking-wider text-slate-400 mb-2">
                Quick Select Global Metropolises
              </div>
              <div className="grid grid-cols-2 gap-2">
                {POPULAR_CITIES.map((city) => (
                  <button
                    ...
                    className="text-left p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-cyan-500/30 transition-all"
                  >
